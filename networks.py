...
class Movement_Enc(nn.Module):
    def __init__(self, pose_size, dim_z_movement, length, hidden_size, num_layers, bidirection=False):
        ...
        self.init_h = nn.Parameter(torch.randn(num_layers * self.num_dir, 1, hidden_size).type(T.FloatTensor),
                                   requires_grad=True)
        if bidirection:
            self.mean = nn.Sequential(
...
class Dance_Enc(nn.Module):
    def __init__(self, dim_z_movement, dim_z_dance, hidden_size, num_layers, bidirection=False):
        ...
        if bidirection:
            ...
            self.num_dir = 1
        self.recurrent = nn.GRU(2 * dim_z_movement, hidden_size, num_layers=num_layers, batch_first=True,
                                bidirectional=bidirection)
        self.init_h = nn.Parameter(torch.randn(num_layers * self.num_dir, 1, hidden_size).type(T.FloatTensor),
                                   requires_grad=True)
        if bidirection:
