...
class Trainer_Decomp(object):
    ...
    def backward_movement_ED(self):
        # z_movement KL
        kl_element = self.z_movement_mus.pow(2).add_(self.z_movement_logvars.exp()).mul_(-1).add_(1).add_(
            self.z_movement_logvars)
        ...
        # Movement dist
        self.loss_dist_z_movement = torch.mean(
            torch.abs(self.z_movement1 - self.z_movement2)) * self.args.lambda_dist_z_movement
    ...
    def train(self, ep=0, it=0):
        ...
        for epoch in range(ep, self.args.num_epochs):
            ...
            for i, (stdpose, stdpose2) in enumerate(self.data_loader):
                ...
                print(
                    'Epoch:{:3} Iter{}/{}\tl_l1_initp {:.3f}\tl_l1_stdp {:.3f}\tl_l1_cross_stdp {:.3f}\tl_dist_zmove {:.3f}\tl_kl_zinit {:.3f}\t l_kl_zmove {:.3f}'.format(
                        epoch, i, len(self.data_loader), self.loss_l1_initp, self.loss_l1_stdp, self.loss_l1_cross_stdp,
                        self.loss_dist_z_movement, self.loss_kl_z_init, self.loss_kl_z_movement))
